...
from qas.span_utils import span_xx_to_cxw
...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, vocab_path, v_feat_dirs, q_feat_dir,
                 q_feat_type="last_hidden_state", vocab_size=None,
        ...
        self.vocab = vocab
        self.keep_vocab = dict() # word index in order
    ...
    def __getitem__(self, index):
        ...
        if self.dset_name == 'hl':
            model_inputs["query_feat"], model_inputs["words_id"], model_inputs["weights"] = \
                self._get_query_feat_by_qid(meta["qid"], meta["query"])
        if self.use_video:
        ...
        if self.use_tef: # positonal encoding
    ...
    def _get_query_feat_by_qid(self, qid, sentence):
        if self.dset_name == 'hl':
        ...
        return words_feat, words_id, weights  # (D, ) or (Lq, D)
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        ...
        if k == "saliency_all_labels":
            ...
            continue
        if k == "query_feat": # three at a time
            bsz = len(batch)
            words_dim = batch[0]["model_inputs"]["query_feat"][0].shape[0]
            words_len = []
            for i, sample in enumerate(batch):
                words_len.append(len(sample['model_inputs']['words_id']))
            
            words_feat = np.zeros([bsz, max(words_len) + 2, words_dim]).astype(np.float32)
            words_id = np.zeros([bsz, max(words_len)]).astype(np.int64)
            weights = np.zeros([bsz, max(words_len)]).astype(np.float32)
            for i, sample in enumerate(batch):
                keep = min(len(sample['model_inputs']['query_feat']), words_feat.shape[1])
                words_feat[i, :keep] = sample['model_inputs']['query_feat'][:keep] # pad with zero
                keep = min(len(sample['model_inputs']['words_id']), words_id.shape[1])
                words_id[i, :keep] = sample['model_inputs']['words_id'][:keep]
                keep = min(len(sample['model_inputs']['weights']), weights.shape[1])
                tmp = np.exp(sample['model_inputs']['weights'][:keep])
                weights[i, :keep] = tmp / np.sum(tmp)
            
            batched_data["query_feat"] = torch.from_numpy(words_feat)
            batched_data["words_id"] = torch.from_numpy(words_id)
            batched_data["weights"] = torch.from_numpy(weights)
            batched_data["words_len"] = torch.from_numpy(np.asarray(words_len))
        
        if k in ["words_id", "weights"]:
            continue
        
        if k not in ["query_feat", "words_id", "weights"]:
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    model_inputs = dict(
        ...
        words_id=batched_model_inputs["words_id"].to(device, non_blocking=non_blocking),
        weights=batched_model_inputs["weights"].to(device, non_blocking=non_blocking),
        words_len=batched_model_inputs["words_len"].to(device, non_blocking=non_blocking),
