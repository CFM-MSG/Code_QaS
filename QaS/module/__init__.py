...
from qas.module.decoder import TransformerDecoder
from qas.module.encoder import TransformerEncoder
...
class VideoEncoder(nn.Module):
    ...
    def forward(self, vid = None, vid_mask  = None, txt = None, txt_mask=None, gauss_weight=None, require_enc = False):

        if vid != None:
            if txt == None:
                vid_enc_out, _ = self.vid_encoder(None, None, vid, vid_mask, tgt_gauss_weight = gauss_weight)
            else:
                vid_enc_out, _ = self.vid_encoder(txt, txt_mask, vid, vid_mask, tgt_gauss_weight = gauss_weight)
            pred = vid_enc_out[:,-1]
            hy = vid_enc_out[:, :-1]
        ...
        if require_enc:
            return vid_concepts, vid_enc_out
        else:
            return vid_concepts, hy
