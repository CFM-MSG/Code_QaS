...
from qas.module.mutihead_attention import MultiheadAttention, CosformerAttention
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=0.0, attn='Vanilla', norm='LayerNorm'):
        ...
        if norm == 'LayerNorm':
            ...
            self.self_attn_layer_norm = nn.BatchNorm1d(d_model)
            self.encoder_attn_layer_norm = nn.BatchNorm1d(d_model)
            self.final_layer_norm = nn.BatchNorm1d(d_model)
    ...
    def forward(self, x, mask, encoder_out=None, encoder_mask=None, self_attn_mask=None, 
        ...
        if self.norm == 'BatchNorm':
            x = self.self_attn_layer_norm(x.permute(1,2,0)).permute(2,0,1)
        else:
        ...
        if encoder_out is not None:
            ...
            x = res + x
            if self.norm == 'BatchNorm':
                x = self.encoder_attn_layer_norm(x.permute(1,2,0)).permute(2,0,1)
            else:
        ...
        x = res + x
        if self.norm == 'BatchNorm':
            x = self.final_layer_norm(x.permute(1,2,0)).permute(2,0,1)
        else:
